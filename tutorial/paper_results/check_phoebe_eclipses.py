...
from spice.models.eclipse_utils import find_eclipses
...
def main(inclination, period, q, ecc, n_times, primary_mass, output_path):
    """
    ...
    def default_icosphere(mass=1.):
        ...
        return get_mesh_view(IcosphereModel.construct(1000, 1., mass, bb.solar_parameters, bb.parameter_names), jnp.array([0., 0., -1.]))
    ...
    filename = os.path.join(output_path, f'eclipses_incl_{inclination}_period_{period}_q_{q}_ecc_{ecc}_primary_mass_{primary_mass}.npz')
    print(f"Saving position and velocity offsets to {filename}")
    import pickle
    with open(filename.replace('.npz', '.pkl'), 'wb') as f:
        pickle.dump({
            'phoebe_binary': b,
            'spice_body1': pb1,
            'spice_body2': pb2,
...
if __name__ == '__main__':
    ...
    main(args.inclination, args.period, args.q, args.ecc, args.n_times, args.primary_mass, args.output_path)
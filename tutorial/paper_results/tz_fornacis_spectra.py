...
@click.option('--emulator', default='RozanskiT/TPayne-spice-harps', help='Pretrained astro_emulators_toolkit intensity bundle')
@click.option('--output-dir', default='data', help='Directory to save output files')
def generate_tz_fornacis_spectra(num_times, num_wavelengths, emulator, output_dir):
    """Generate synthetic spectra for TZ Fornacis binary system."""
    click.echo(f"Loading pretrained intensity emulator '{emulator}'...")
    em = IntensityPretrainedAemuSpectrumEmulator(emulator)

    click.echo("Creating primary star model (G-type giant)...")
    body1 = IcosphereModel.construct(500, 8.28, 2.057, em.to_parameters(dict(teff=4930, logg=2.91)), em.stellar_parameter_names, override_log_g=False)

    click.echo("Creating secondary star model (F-type subgiant)...")
    body2 = IcosphereModel.construct(500, 3.94, 1.958, em.to_parameters(dict(teff=6650, logg=3.35)), em.stellar_parameter_names, override_log_g=False)

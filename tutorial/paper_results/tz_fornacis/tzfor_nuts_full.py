...
ap.add_argument("--out", required=True); ap.add_argument("--resume", action="store_true", help="continue from the chains already saved in --out");
ap.add_argument("--chains-per-run", type=int, default=None, help="sample at most this many chains in this process, then exit (the job script loops with --resume)"); ap.add_argument("--n-epochs", type=int, default=None); ap.add_argument("--n-lc-epochs", type=int, default=None)
args = ap.parse_args()
...
# Even so, the SECOND mcmc.run of a process always failed to load its first kernel (CUBIN
# out-of-memory), with the pool at 19 of 40 GB and capped at 80%: it is not device memory the
# pool holds. Every chain is therefore saved as soon as it is done, and the job script runs one
# chain per process (--chains-per-run 1 --resume) until --chains are in the file.
...
if len(chains) < args.chains:
    print(f"NUTS: {len(chains)}/{args.chains} chains in {args.out}; rerun with --resume for the rest", flush=True); sys.exit(0)

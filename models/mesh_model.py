...
from .mesh_generation import icosphere, calculate_rotation
...
class StarModel(metaclass=ABCMeta):
    ...
    @vertices.setter
    def vertices(self, vertices: jnp.ndarray):
        self.__set_vertices(vertices)
    
    @abstractmethod
    def __set_vertices(self, new_vertices):
        raise NotImplementedError
    ...
    def faces(self, faces: jnp.ndarray):
        self.__set_faces(faces)
    
    @abstractmethod
    def __set_faces(self, new_faces):
        raise NotImplementedError
    ...
    @abstractmethod
    def __set_centers(self, new_centers: jnp.ndarray):
        raise NotImplementedError
    ...
    @abstractmethod
    def get_velocities_vectors(self) -> jnp.ndarray:
        raise NotImplementedError
...
class MeshModel(StarModel):
    def __init__(self, n_vertices: int, radius: float, phases: jnp.array, timestamps: jnp.array):
        ...
        self.__verts = jnp.repeat(verts[:, jnp.newaxis], self.__phases.shape[0], axis=0)
        self.__faces = jnp.repeat(faces[:, jnp.newaxis], self.__phases.shape[0], axis=0)
        self.__areas = jnp.repeat(areas[:, jnp.newaxis], self.__phases.shape[0], axis=0)
        self.__centers = jnp.repeat(centers[:, jnp.newaxis], self.__phases.shape[0], axis=0)

        self.__radii = jnp.zeros_like(self.__centers)
    ...
    def apply_rotation(self, omega: float,
        ...
        self.centers, self.__velocities, self.__radii = calculate_rotation(self.__omega,
                                                                           self.__rotation_axis,
                                                                           self.centers,
                                                                           self.timestamps)
        return jnp.vstack([self.centers, self.__velocities])
